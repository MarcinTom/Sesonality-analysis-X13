...
def x13_arima_analysis(endog, maxorder=(2, 1), maxdiff=(2, 1), diff=None,
    ...
    spec += "x11{ save=(d10 d11 d12 d13) }" #adding seasonal component info
    if speconly:
        return spec
    ...
    try:
        ...
        factor = _open_and_read(ftempout.name + '.d10') #adding seasonal component info
    ...
    factor = _convert_out_to_series(factor, endog.index, 'factor') #adding seasonal component info
    ...
    if not retspec:
        res = X13ArimaAnalysisResult(observed=endog, results=results,
                                     seasadj=seasadj, factor=factor, trend=trend,
...
class X13ArimaAnalysisResult(object):
    ...
    def plot(self):
        ...
        self.factor.plot(ax=axes[4], legend=False)
        axes[4].set_ylabel('Factor')
